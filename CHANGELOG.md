...
- `getNamedDestination(name)` on `PDFiumDocument`, resolving a single anchor by name and returning `null` when the document carries none with that name. It exists because `getNamedDestinations()` cannot always enumerate what the document declares: `FPDF_GetNamedDest` resolves an index below the `/Names /Dests` name-tree count through the tree, which dereferences indirect objects, and lets the rest fall through to the legacy catalog `/Dests` dictionary, which does not — so a legal entry whose value is an indirect reference (ISO 32000-1 allows one wherever a direct object is allowed) is counted by `FPDF_CountNamedDests` and then yields nothing. Not a destination, and not a name either: the in/out buffer length is left at zero, which is why such an entry cannot be surfaced as a `null` the way the other counted listings surface theirs. `FPDF_GetNamedDestByName` searches both stores and dereferences in either, so it resolves exactly the destinations the listing drops. Resolving the `#Chapter2` of an incoming link is the stated use of this part of the API, and for such a document the listing previously returned nothing to match the name against. Pinned by a new `indirect-legacy-dests.pdf` fixture whose three legacy entries are counted and unlistable while its one name-tree entry lists normally.
- `metadata.namedDestinationCount`, the `FPDF_CountNamedDests` total. `getNamedDestinations()` is the one listing in the library that can come back shorter than its own count, and nothing previously exposed the count, so the shortfall was invisible — the same silent-drop shape as [#34](https://github.com/xonaman/nodejs-pdfium-native/issues/34), which the fix for that issue could not reach here because there is no index to hang a `null` on. A caller can now compare `getNamedDestinations().length` against it and fall back to `getNamedDestination(name)`.

### Fixed

- `getAttachments()` silently dropped an embedded file that failed to load, contradicting `metadata.attachmentCount` with no way for a caller to tell ([#34](https://github.com/xonaman/nodejs-pdfium-native/issues/34)). Both numbers come from the same `FPDFDoc_GetAttachmentCount` call, but the count only sums the slots in the `/EmbeddedFiles` name tree while `FPDFDoc_GetAttachment` resolves each one and is documented to return `NULL` _on failure_ — so a filespec reference pointing at a missing object is counted and does not load. The loop mapped that onto `continue`, which is the same outcome as the attachment not existing: a document reporting `attachmentCount: 1` came back from `getAttachments()` as `[]`. That is the dangerous direction to be wrong in, because an empty array reads as "safe to proceed" — `splitDocument()` and `mergeDocuments()` discard the embedded-files tree, so a caller checking for attachments before splitting would destroy the file it was checking for, and for a PDF/A-3 e-invoice the embedded XML is usually the only machine-readable copy. The failed index is now reported as a `null` entry, so the array length always matches the count and array position is the document index. Pinned by a new `damaged-attachment.pdf` fixture. Note the sibling `getAttachment(index)` already rejected for the same index, so the list path was inconsistent with the read path.
- The same skip-on-`NULL` shape was silently losing entries in three more listings, each now reporting a `null` the same way, each with its own damaged fixture: `getAnnotations()` (`dangling-annotation.pdf`), `getJavaScriptActions()` (`dangling-javascript.pdf`) and `getFormFields()` (`damaged-widget.pdf`). `getFormFields()` is the awkward one: it is a filtered subset — only widget annotations are form fields — and the subtype is readable only through the handle that failed, so an unloadable annotation cannot be ruled out as a non-widget. Dropping it let a damaged widget pass for a page that simply has fewer fields; reporting it means a `null` there says "a form field _may_ be missing", and the fixture deliberately damages one widget and one plain annotation so that the indistinguishability is exercised rather than merely documented. Since that listing carries no index, `getAnnotations()` on the same page reports the identical failure at its true index.
- `getSignatures()` got the same treatment for consistency with its twin, though no document was found that reaches it. Thirteen damage shapes were tried and `metadata.signatureCount` and the array length moved in lockstep every time; both entry points appear to derive from the same collection of signature dictionaries. This one is defensive against the documented `"NULL on failure"` contract and a bundled PDFium that gets bumped, not a fix for observed loss — and is consequently the one site with no regression test.

### Changed

- **Breaking (types).** The five listing methods above now return `(T | null)[]` rather than `T[]`. Nothing changes at runtime for a well-formed document — every entry is non-`null`, and every existing fixture produces identical results — but under `strictNullChecks` the compiler will flag call sites that assumed success. That is the point: the previous behaviour was invisible by construction, and a flag or a side-channel count would have been just as ignorable as the silent drop it replaced. Use `a?.name`, or filter with `(a) => a !== null` where a `null` is genuinely acceptable.
- A `null` is only reported once PDFium has counted the entry, so damage severe enough to break the count is still invisible: an `/EmbeddedFiles` name tree that fails to parse at all reports `attachmentCount: 0` and an empty array, indistinguishable from a document with no embedded files. This is the scope the issue itself set, and it is now stated in the README rather than left implicit. For the page-level listings the trigger is also narrower than "damage" — `FPDFPage_GetAnnot` returns `NULL` for an explicit PDF null object left in an `/Annots` array, which is legal and something some producers emit after deleting an annotation.
- Three neighbouring `NULL` skips were deliberately **not** converted, and each now carries a comment saying why. `FPDF_StructElement_GetChildAtIndex` returns `NULL` for a child that is a content reference rather than an element, which the code already documented. `FPDF_StructTree_GetChildAtIndex` turns out to do the same for a top-level element belonging to a **different page** — the count is document-wide while `getStructTree()` walks one page — so a document with one `/Part` per chapter yields a `NULL` for every chapter that is not the current one; the existing fixtures hid this because one is single-root and the other untagged. `getNamedDestinations()` was converted and then reverted after review: `FPDF_GetNamedDest` resolves indices below the `/Names /Dests` name-tree count through the tree, which dereferences indirect objects, and lets the rest fall through to the legacy catalog `/Dests` dictionary, which does not, while `FPDF_CountNamedDests` counts every legacy key regardless — so a legal entry whose value is an indirect reference to a destination array is counted, returns `NULL`, and would have been reported as damage. A three-entry legacy `/Dests` dictionary with indirect values came back `[null, null, null]` while PDFium resolved all three correctly through its own name lookup. PDFium exposes no way to ask for the name-tree count alone, so the two cases cannot be told apart at that call site. `getNamedDestinations()` keeps its `NamedDestination[]` type and its existing drop behaviour.
